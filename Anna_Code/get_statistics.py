...
def overall_packet_distribution(df):
    """
        Summarize total, normal, and attack packet distribution.
    """
    total = len(df)

    # Map attack labels to category names- "nicer labels"
    labels = []
    for value in df['label_attack']:
        if value == 1:
            labels.append('Attack')
        else:
            labels.append('Normal')

    attack_series = pd.Series(labels, name='packet_type')

    # Compute counts, fractions, and percentages
    total_packet_stats = counts_fractions_helper(attack_series, total).sort_values('count', ascending=False)
    ...
    total_packet_stats = pd.concat([total_row, total_packet_stats])

    return {'total_packet_stats': total_packet_stats}
...
def print_packet_distribution_task1A(df):
    ...
    print("Task 01A++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++")
    print(f" Total number of packets (attack and control set): \n{total_results['total_packet_stats'].to_string()}")


    print(f"\nNumber of distinct app-layer protocols (excl. 'none'): "
          f"{app_results['num_app_protocols_excl_none']}")
    print(f"\nProtocols: {', '.join(app_results['app_protocol_list_excl_none'])}\n")
    print("\nComplete Dataset:")
...
def print_packet_length_distribution_and_iat_task1B(df):
    ...
    print("""
    Only consider packets with application data.
    Compute mean, std, and median of frame_len (packet size in bytes) for each protocol type
    ...
    print("""
    Computes average, average, std, and median inter-arrival time between same host pair for normal packets and attack packets subset.
    """)
...
def print_packet_distribution_task1C(df):
    df=preprocess(df)
    print("\nTask 01C++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++")
    print("""\nHow many pairs of hosts for complete dataset and attack subset.""")
