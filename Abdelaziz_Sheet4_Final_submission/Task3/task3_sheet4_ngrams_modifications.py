...
def load_and_label_data(p):
    '''
        read data from final_data/
            normal: final_processed_packets_{p}_dedup_normal.npy
            attack: final_processed_packets_{p}_dedup_attack.npy
        Assign label for them: normal 0, attack 1
        return labeled_data_normal, labeled_data_attack
    '''
    normal_data_path = f'final_data/final_processed_packets_{p}_dedup_normal_data.npy'
    attack_data_path = f'final_data/final_processed_packets_{p}_dedup_attack_data.npy'
    labeled_data_normal = []
    labeled_data_attack = []
    print(f"[*] Loading normal packets from {normal_data_path}...")
    if os.path.exists(normal_data_path):
        normal_packets = np.load(normal_data_path, allow_pickle=True)
        for pkt in normal_packets:
            labeled_data_normal.append( (pkt, 'normal') )
    else:
        raise FileNotFoundError(f"Normal data file not found: {normal_data_path}")
    print(f"[*] Loading attack packets from {attack_data_path}...")
    if os.path.exists(attack_data_path):
        attack_packets = np.load(attack_data_path, allow_pickle=True)
        for pkt in attack_packets:
            labeled_data_attack.append( (pkt, 'attack') )
    else:
        raise FileNotFoundError(f"Attack data file not found: {attack_data_path}")
    return labeled_data_normal, labeled_data_attack


def compute_bloom_weights_and_counts(train_packets, n,p):
    
    print("[*] Training model...")
    if os.path.exists(f'bloom_filter_n_{n}_p_{p}.pkl') and os.path.exists(f'ngram_weights_n_{n}_p_{p}.pkl') and os.path.exists(f'ngram_count_training_n_{n}_p_{p}.pkl'):
        
        with open(f'bloom_filter_n_{n}_p_{p}.pkl', 'rb') as f:
            bloom = pickle.load(f)
        with open(f'ngram_weights_n_{n}_p_{p}.pkl', 'rb') as f:
            weights = pickle.load(f)
        with open(f'ngram_count_training_n_{n}_p_{p}.pkl', 'rb') as f:
        ...
        with open(f'bloom_filter_n_{n}_p_{p}.pkl', 'wb') as f:
            pickle.dump(bloom, f)
        with open(f'ngram_weights_n_{n}_p_{p}.pkl', 'wb') as f:
            pickle.dump(weights, f)
        # store ngram_count_training for future use
        with open(f'ngram_count_training_n_{n}_p_{p}.pkl', 'wb') as f:
...
def sheet3_task1():
    ...
    for p in [5,10,15]:
        ...
        labeled_data_normal, labeled_data_attack = load_and_label_data(p) 
        ...
        bloom, weights = compute_bloom_weights_and_counts(train_packets, n,p)
        ...
        with open(f'validation_scores_n_{n}_p_{p}.txt', 'w') as f:
        ...
        with open(f'best_metrics_n_{n}_p_{p}.txt', 'w') as f:
        ...
        with open(f'test_results_n_{n}_p_{p}.txt', 'w') as f:
            f.write("Index\tScore\tLabel\n")
