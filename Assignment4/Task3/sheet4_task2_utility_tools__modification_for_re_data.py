...
def phase1_getting_data(n,m,p):
    ...
    Data_Files_Exist = os.path.exists('./final_data/training_data.npy') and os.path.exists('./final_data/validation_data.npy') and os.path.exists('./final_data/testing_data.npy') and os.path.exists('./final_data/training_labels.npy') and os.path.exists('./final_data/validation_labels.npy') and os.path.exists('./final_data/testing_labels.npy')
    if Data_Files_Exist:
        # load them
...
def phase6_discriminator_mode(
    ...
    with open(f"metrics_D_{p}.txt", "w") as f:
...
def phase6_generator_mode(
    ...
    with open(f"metrics_G_{p}.txt", "w") as f:
        f.write(f"Precision: {precision:.4f}\n")
        f.write(f"Recall: {recall:.4f}\n")
        f.write(f"F1: {f1:.4f}\n")
