...
{\color{red} In this chapter we present....}


{\color{red} todo: `always motivate and discuss why a given step is important to solve your scientific problem'}
...
You can find all implementations in our  repository on github: \url{https://github.com/abdelazizSalah/Study_Project}, and we will keep some classes implementations in the Apendix as well. 
...
and Reverse-Engineered (RE).

\subsubsection{Label Extraction and Metadata Processing}
...
\subsubsection{RAW Dataset Representation}
...
including Ethernet, IP, and TCP/UDP headers followed by the
application-layer payload (if present). No additional parsing or
content truncation was performed in this representation.

\subsubsection{Reverse-Engineered (RE) Dataset Representation}

For the Reverse-Engineered (RE) representation, only S7Comm-related
application-layer data was considered. Packets were filtered according
to the following criteria:

\begin{itemize}
    \item The packet contains a TCP layer,
...
The keyword is identified in the Reverse Engineering process, as we describe in the following section \cref{sec:rev-eng}. 
This representation therefore focuses on protocol-relevant content
rather than the full packet structure.

\subsubsection{Concatination of Bytes to create RE5,RE10,RE15}
%ToDo Abdelaziz
{\color{red} todo}
...
multiple messages.
{\color{red} todo how did we do the progressive sequence alignment}
...
{\color{gray} 
\paragraph{Filtering Static and Almost-Static Candidates}
Candidates that produce only a single cluster are treated as static and are excluded, since
they do not provide a meaningful partitioning of messages. In addition, we filter
\emph{almost-static} candidates, defined as candidates for which the largest cluster contains
at least a fraction $\tau$ of all messages (in our implementation $\tau = 0.9$). Such candidates
typically indicate fields that vary only rarely and are therefore unlikely to represent a
useful keyword boundary for the start of physical readings.

\paragraph{Cluster Compactness Statistics}
For the remaining candidates, we compute descriptive statistics of the induced partitioning,
including the number of clusters, the average cluster size, minimum and maximum cluster size,
and the fraction of singleton clusters (clusters of size one). The singleton fraction serves as
an indicator of fragmentation: a high number of singleton clusters suggests that the candidate
field produces overly fine-grained partitions (potentially random variation), whereas more
compact clusterings indicate structured variability that is consistent across messages.}

\paragraph{Payload-Length Homogeneity within Clusters}
...
{\color{red} todo: we need to conclude which approach we finally used and maybe what keyword we finally chose}
...
{\color{gray} The dataset is divided into $k$ equally sized folds.
...
As illustrated in \cref{fig:kfolds}, in each iteration the validation
...
to determine optimal parameter configurations.
...
Instead, we report {\color{red} precision, recall, and} the F1-score,
which provide a more informative assessment under skewed
class distributions.
...
For the binary linear Support Vector Machine (SVM), feature importance is computed
...
For all other traditional ML classifiers, permutation-based feature importance
...
\paragraph{Error Overlap Analysis}
...
\paragraph{Runtime and Memory Consumption}
...
\begin{itemize}
    ...
    \item Average training time and average inference time for each traditional ML detection method,
...
%ToDo Anna
\subsubsection{Feature Extraction using the Stacked Denoising Autoencoder (SDA)}
Traditional ML classifiers such as SVM, kNN, and Random Forest require a fixed-length, low-dimensional feature representation and do not natively handle variable-length byte sequences. Therefore we employ a \ac{SDA} as a feature extractor to obtain compact latent representations of the preprocessed
...
For each dataset representation, the SDA is trained exclusively on benign samples and on the training set from the current k-fold iteration. Using only benign samples enables the
model to learn a compressed representation of normal traffic patterns.

After training, the SDA is used to encode all samples of the corresponding representation.
...
{\color{red} todo: do we need to include the implementation of each of the traditional ML classifiers?}
...
As illustrated in \cref{fig:ensemble-structure} the ensemble classifier combines the predictions of three independently
...
{\color{red} todo: why do we use DL detection methods}
\subsubsection{CNN}
%ToDo Anna

We implemented a Convolutional Neural Network (CNN) to perform binary attack detection directly on the preprocessed byte sequences (i.e., without using the autoencoder-generated features). Similar to the feature generation pipeline, all
...
\subsubsection{\ac{ResNet}} is a deep neural network architecture designed to enable the effective training of very deep models by introducing residual (skip) connections between layers. Traditional deep neural networks often suffer from vanishing or degrading gradients as the number of layers increases, making optimization difficult and sometimes leading to reduced performance when the network becomes deeper. ResNet addresses this issue by reformulating the learning objective: instead of directly learning a desired mapping $H(x)$ from input $x$, each residual block learns a residual function $F(x) = H(x) - x$, and the final output of the block is computed as $H(x) = F(x) + x$. This identity-based skip connection allows gradients to propagate more easily through the network during backpropagation, thereby stabilizing training and enabling the construction of substantially deeper architectures. A typical ResNet architecture consists of multiple stages, each containing several residual blocks composed of convolutional layers, batch normalization, and non-linear activation functions such as ReLU. In the context of anomaly detection in Industrial Control Systems (ICS), ResNet can be adapted to process one-dimensional byte sequences of network packets using one-dimensional convolutional layers. The residual connections help preserve low-level structural information while progressively extracting higher-level representations, which improves classification performance on complex and high-dimensional network traffic data. Due to its stable training behavior and strong representational capacity, ResNet is particularly suitable for modeling subtle deviations in proprietary binary communication protocols. ResNet accepts as an input \textit{m} x \textit{n} complete network packets or the parsed physical readings, where m is the number of packets, and n is the number of bytes per each packet. 
...
{\color{red} todo: why do we use them}
\subsubsection{SDA-Based Reconstruction Error Detection}

The implemented stacked denoising autoencoder (SDA)
is employed as a standalone anomaly detection method based on
reconstruction error.

\paragraph{Underlying Idea}

The SDA is trained exclusively on benign network traffic. During training,
...
\paragraph{Reconstruction Error Computation}
...
error is computed using the Mean Squared Error (MSE):
...
To transform the SDA into a binary classifier, a decision threshold
$\tau$ is defined based on the reconstruction error distribution of
benign training samples. Concretely, we compute the per-sample MSE
...
During testing, each packet is processed by the trained SDA and its
...
\] 
Score \in [0, 1], where   $N_{\text{seen}_i}$
...
properties and minimizing correlations between generated hash values.
