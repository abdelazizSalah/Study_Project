...
First, we define a maximum keyword length constraint. This constraint introduces a parameter \textit{L}, which represents the maximum allowed length of a keyword. Any candidate keyword whose length exceeds \textit{L} is discarded. This step reduces the number of potential candidates and removes fields that are unlikely to represent protocol keywords.
...
The \ac{SDA} is trained exclusively on benign network traffic. During training,
