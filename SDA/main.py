...
from SDA.autoencoder import hyperparameter_search
...
def create_all_models_extract_features(output_dir, ds_train, ds_test):

    #create 6 models: 2 different layers, 3 differnt activation function
    model_dense_relu, param_settings = hyperparameter_search(ds_train, ds_test, activation="relu", layer_type="dense")
    ...
    model_dense_elu, param_settings = hyperparameter_search(ds_train, ds_test, activation="elu", layer_type="dense")
    extract_and_print_features_to_file(ds_train, "dense_elu", model_dense_elu, f"Model 2: Dense Layers, elu activation functions.\n Hyperparameters: {param_settings})")


    model_dense_tanh, param_settings = hyperparameter_search(ds_train, ds_test, activation="tanh", layer_type="dense")
    extract_and_print_features_to_file(ds_train, "dense_tanh", model_dense_tanh, f"Model 3: Dense Layers, tanh activation functions.\n Hyperparameters: {param_settings})")


    model_conv1d_relu, param_settings = hyperparameter_search(ds_train, ds_test, activation="relu", layer_type="conv1d")
    ...
    model_conv1d_elu, param_settings = hyperparameter_search(ds_train, ds_test, activation="elu", layer_type="conv1d")
    extract_and_print_features_to_file(ds_train, "conv1d_elu", model_conv1d_elu, f"Model 3: conv1D Layers, elu activation functions.\n Hyperparameters: {param_settings})")

    model_conv1d_tanh, param_settings = hyperparameter_search(ds_train, ds_test, activation="tanh", layer_type="conv1d")
