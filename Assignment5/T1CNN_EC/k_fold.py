...
def create_folds_pure_python(data_indices, k):
    ...
    #e.g. 1/5
    fold_size = num_samples // k  #//->rounds results down to the nearest integer
    ...
    for i in range(k):
        ...
        if i<remainder: #compare index of fold with remainder (e.g. 3 < 2)
            current_fold_size = fold_size + 1   # add an extra index to fold (distribute remainder)
        else:
            current_fold_size = fold_size + 0

        # cut fold out of list with indices
...
def create_folds_for_all_attack_types(labels, k):
    ...
    attack_folds = {
        "A": [fold0, fold1, ..., fold_k-1], (sublists)
...
def scenario1(labels, k):
    ...
    folds_only_control=create_folds_pure_python(control_indices,k) # [[],[]] sublist for each control fold
...
def scenario2(labels, k):
    ...
    # for each fold 2 random attack types removed (training)
    for fold_idx in range(k):
